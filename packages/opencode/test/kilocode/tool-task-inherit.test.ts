...
describe("Kilo task inheritDeny option", () => {
  ...
  test("KiloTask.inherited returns empty array when inheritDeny is false", () => {
    // Direct unit test of the inherited function
    const { KiloTask } = require("../../src/kilocode/tool/task")
  ...
  test("deriveSubagentSessionPermission respects inheritDeny: false", () => {
    const { deriveSubagentSessionPermission } = require("../../src/agent/subagent-permissions")

    const parentSessionPermission: Permission.Ruleset = [
      { permission: "bash", pattern: "*", action: "deny" },
      { permission: "edit", pattern: "*", action: "deny" },
    ]
    const parentAgentPermission: Permission.Ruleset = [
      { permission: "edit", pattern: "*", action: "deny" },
    ]
    ...
    const resultFalse = deriveSubagentSessionPermission({
      ...
      parentAgent: { name: "build", permission: parentAgentPermission, mode: "primary", options: {} },
      subagent: {
        name: "worker",
        permission: [],
        mode: "subagent",
    ...
    // With inheritDeny: true - should include parent deny rules
  ...
  test("KiloTask.inherited only filters for edit, bash, and MCP permissions", () => {
    const { KiloTask } = require("../../src/kilocode/tool/task")

    // Non-MCP tool permissions should NOT be inherited even if deny
    const callerPermission: Permission.Ruleset = [
      { permission: "bash", pattern: "*", action: "deny" as const },
      { permission: "edit", pattern: "*", action: "deny" as const },
    ...
    // Should only include bash, edit, and MCP deny rules
    expect(result).toContainEqual({ permission: "bash", pattern: "*", action: "deny" })
